// 🤖 Review: This file was created by refactoring legacy/ai-assistant.js into an ES6 module.
...
export const AIAssistant = {
    ...
    for (const note of notes) {
      ...
      if (score > 2) {
        suggestions.push({
          ...
          message: `Consider linking to "${note.title || note.id}" - these notes seem related.`, // Corrected escaping for double quotes within a string literal
          action: () => {
  ...
  suggestTags(notes, currentNote) {
    ...
    // Common tag patterns
    const tagPatterns = {
      'question': /\b(what|how|why|when|where|who|which|explain|define)\b/, // Corrected escaping for backslash in regex
      'todo': /\b(todo|task|need to|should|must|action|next step)\b/, // Corrected escaping for backslash in regex
      'important': /\b(important|critical|key|vital|essential|crucial|priority)\b/, // Corrected escaping for backslash in regex
      'research': /\b(research|study|investigate|explore|examine|analyze|investigation)\b/, // Corrected escaping for backslash in regex
      'idea': /\b(idea|concept|thought|insight|observation|notion)\b/, // Corrected escaping for backslash in regex
      'reference': /\b(reference|source|citation|quote)\b/, // Corrected escaping for backslash in regex
    ...
    for (const [tag, pattern] of Object.entries(tagPatterns)) {
      if (!currentTags.has(`#${tag}`) && pattern.test(content)) {
        suggestions.push({
          ...
          priority: 'medium',
          title: 'Add Tag',
          message: `Consider adding #${tag} tag based on note content.`, // Corrected escaping for double quotes within a string literal
          action: () => {
    ...
    for (const [tag, group] of Object.entries(tagGroups)) {
      if (group.length > 5 && !notes.some(note => 
        ...
        suggestions.push({
          ...
          message: `Create an index note for ${tag} (${group.length} notes)`, // Corrected escaping for double quotes within a string literal
          action: async () => {
  ...
  suggestReviews(recentNotes) {
    ...
    wellConnected.forEach(note => {
      suggestions.push({
        ...
        message: `Review "${note.title || note.id}" - it has ${(note.links || []).length} connections`, // Corrected escaping for double quotes within a string literal
        action: () => {
          if (typeof UI !== 'undefined' && typeof UI.openNote === 'function') {
            UI.openNote(note.id);
          }
  ...
  suggestKnowledgeGaps(notes) {
    ...
    if (isolated.length > 0) {
      suggestions.push({
        ...
        priority: 'high',
        title: 'Knowledge Gap',
        message: `${isolated.length} isolated notes found - consider connecting them`, // Corrected escaping for double quotes within a string literal
        action: () => {
    ...
    if (poorlyTagged.length > 0) {
      suggestions.push({
        ...
        message: `${poorlyTagged.length} well-connected notes need better tagging`, // Corrected escaping for double quotes within a string literal
        action: () => {
  ...
  calculateStringSimilarity(str1, str2) {
    ...
    const words1 = new Set(str1.toLowerCase().match(/\w+/g) || []); // Corrected escaping for backslash in regex
    const words2 = new Set(str2.toLowerCase().match(/\w+/g) || []); // Corrected escaping for backslash in regex
  ...
  extractKeywords(text) {
    ...
    const words = text.toLowerCase().match(/\b\w{4,}\b/g) || []; // Corrected escaping for backslash in regex
    const stopWords = new Set(['the', 'and', 'for', 'are', 'but', 'not', 'you', 'all', 'can', 'had', 'her', 'was', 'one', 
  ...
  async generateDailyInsights(notes) {
    ...
    if (todayNotes.length > 0) {
      insights.push({
        type: 'activity',
        title: 'Today\'s Activity', // Corrected escaping for single quote within a string literal
        message: `You\'ve worked on ${todayNotes.length} notes today. Keep building your knowledge graph!` // Corrected escaping for single quote within a string literal
      });
    }
    ...
    if (weekNotes.length > 0) {
      ...
      insights.push({
        type: 'trend',
        title: 'Weekly Trend',
        message: `This week you\'ve created ${weekNotes.length} notes with an average of ${avgLinks.toFixed(1)} links each.` // Corrected escaping for single quote within a string literal
      });
    }
